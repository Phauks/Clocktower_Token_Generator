...
import { assetStorageService } from '../../services/upload/AssetStorageService';
...
export function StudioView() {
  ...
  useEffect(() => {
    ...
    const loadPendingOperation = async () => {
      try {
        console.log('[StudioView] Loading pending operation:', pendingOp.type, pendingOp.metadata, 'editMode:', pendingOp.editMode);
        ...
        if (pendingOp.type === 'loadFromBlob' && pendingOp.data instanceof Blob) {
          ...
          if (isAssetReference(url)) {
            console.log('[StudioView] Loading from AssetReference:', url);
            ...
            console.log('[StudioView] Fetching from URL:', proxiedUrl.substring(0, 50) + '...');
          ...
          console.log('[StudioView] Loading asset by ID:', pendingOp.data);
        ...
        console.error('[StudioView] Failed to load pending operation:', error);
