...
import { createTokensZip } from './pdfGenerator.js'
...
export async function regenerateCharacterAndReminders(
  ...
  try {
    ...
    const charFilename = editedCharacter.name.replace(/[^a-zA-Z0-9]/g, '_')
    ...
    if (editedCharacter.reminders && Array.isArray(editedCharacter.reminders)) {
      ...
      for (const reminder of editedCharacter.reminders) {
        try {
          ...
          const reminderBaseName = `${editedCharacter.name}_${reminder}`.replace(/[^a-zA-Z0-9]/g, '_')
