...
export function CharacterGridView({ characters, tokens }: CharacterGridViewProps) {
  ...
  const charactersByTeam = useMemo(() => {
    ...
    for (const char of characters) {
      ...
      const validTeam = TEAM_ORDER.includes(normalizedTeam as Team)
        ? (normalizedTeam as Team)
        : 'townsfolk';
  ...
  }, [characters, getCharacterTokenVariants, imageDataUrls.has]);
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        <h3 className={styles.title}>📚 CHARACTERS ({characters.length})</h3>
      </div>
        ...
        {TEAM_ORDER.map((team) => {
          ...
          return (
            <div key={team} className={`${styles.category} ${teamClassName}`}>
              ...
              >
                <span className={styles.categoryIcon}>{isCollapsed ? '▶' : '▼'}</span>
