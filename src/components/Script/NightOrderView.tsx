...
import { exportNightOrderPDF } from '../../ts/nightOrder/nightOrderWysiwygExport.js'
...
export function NightOrderView({ enableDragDrop = true, activeTab, onTabChange, onEditCharacter }: NightOrderViewProps) {
    ...
    const handleExportPDF = useCallback(async () => {
        ...
        try {
            ...
            // Use WYSIWYG export - captures DOM exactly as rendered
            await exportNightOrderPDF(
                firstNightRef.current,
                otherNightRef.current,
                filename,
                { dpi: generationOptions.dpi! }
            )
        } catch (err) {
    ...
    }, [displayMeta, isExporting, generationOptions.dpi])
        ...
        <ViewLayout variant="2-panel">
            ...
            <ViewLayout.Panel position="left" width="left" scrollable>
                <div className={layoutStyles.panelContent}>
                    ...
                    {/* Export Section */}
                    <details className={layoutStyles.sidebarCard} open>
                        <summary className={layoutStyles.sectionHeader}>Export</summary>
                        <div className={layoutStyles.optionSection}>
                            <div className={styles.buttonGroup}>
                                <button
                                    className={styles.exportButton}
                                    onClick={handleExportPDF}
                                    disabled={isExporting || hasNoData || !generateNightOrder}
